...
int main(void)
{
    ...
    int codigosDestinos;
    char codigosDestinosTexto[4][4] = {"BRA", "MDQ", "MZA", "BRC"}; //Codigos.
    char nombresDestinos[4][30] = {"Brasil", "Mar del Plata", "Mendoza", "Bariloche"};//Nombre de los destinos.
    float precios[4] = {25000.0, 14000.0, 19000.0, 23000.0}; //Precios de los destinos.
    int capacidades[4] = {0, 0, 0, 0}; //Contador por destino.

    char codigo[240][4]; //Guarda el codigo de destino, cada pasajero tiene su propio codigo.
    char dni[240][10]; //Guarda el DNI com cadena de texto, para validarlo por caracter.
    char apellido[240][50]; //Guarda el apellido del pasajero.
    char nombre[240][50]; //Guarda el nombre del pasajero.
    int edad[240]; //Guarda la edad del pasajero.
    int pagaTarjeta[240]; //Indica si el pasajero paga con tarjeta.
    float importeFinal[240]; //Importe que se debe pagar. Se calcula con base, edad y si paga con tarjeta.
    ...
    for (i = 0; i < n; i++)
    {
        ...
        } while (codigosDestinos < 1 || codigosDestinos > 4);

        if (capacidades[codigosDestinos - 1] < 60)
        {
        ...
        do
        {
            dniValido = 1; //Asumimos que el DNI es valido.

            printf("Ingrese el DNI: ");
            scanf("%s", dni[i]);

            int largo = strlen(dni[i]);
            ...
            for (j = 0; j < largo; j++)
            {
                if (dni[i][j] < '0' || dni[i][j] > '9') //Los digitos tiene que estar en 0 y 9.
                {
                    printf("DNI inválido. Debe contener solo números.\n");
                    dniValido = 0; //Si marca invalido, se repite el ciclo.
                    break;         //Salir del bucle, ya que no hace falta seguir validando más.
                }
            }
